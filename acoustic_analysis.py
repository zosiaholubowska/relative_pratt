...
fig.suptitle('Comparison of Spectral Features')
fig.set_figheight(10)
fig.set_figwidth(15)

for idx, feature in enumerate(features):
    data = acoustic_features_df[acoustic_features_df['feature'] == feature]
    sns.stripplot(data=data, x='condition', y='value', hue='midi', hue_norm=(-100, 100), ax=axs[idx], palette='Spectral')
    sns.boxplot(data=data, x='condition', y='value', fill=False, color='black', ax=axs[idx])
    axs[idx].set_title(feature)

plt.show()

plt.savefig(f'{PLOT_DIR}/acoustic_features_stimuli.png', dpi=300)
